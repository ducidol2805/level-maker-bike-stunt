#!/usr/bin/env python3
"""Shared scene compiler and preview renderer for Map Viewer and Library Studio."""
...
def platform_style(group: str, shape: dict[str, Any], outline_only: bool = False
                   ) -> tuple[str, str, str, float]:
    """One fill/stroke policy for open and closed platforms in both apps."""
    closed = shape.get('closed', False)
    if group == 'FreePlatform':
        name, fill, outline = 'Free Platform', '#3b2a57', '#a78bfa'
    else:
        name, fill, outline = 'RampPlatform', '#573a20', '#d97706'
        phase = shape.get('metadata', {}).get('routePhase')
        if phase == 'before_explosion':
            name, fill, outline = 'Pre-explosion route', '#574020', '#f59e0b'
        elif phase == 'after_explosion':
            name, fill, outline = 'Revealed route', '#164652', '#22d3ee'
    return name, '' if outline_only or not closed else fill, outline, 3 if closed else 5


def shape_line_color(group: str, shape: dict[str, Any]) -> str:
    """Readable editor lines using the same category colors as the preview legend."""
    if group in ('RampPlatform', 'FreePlatform'):
        return platform_style(group, shape)[2]
    label = {'MainPlatform': 'MainPlatform terrain', 'deadzone': 'Deadzone',
             'InteractableObject': 'Explosive ramp'}.get(group, group)
    return LEGEND_STYLES.get(label, (UI_MUTED, ''))[0]
...
def compile_vector_level(data: dict[str, Any], outline_only: bool = False,
    ...
    for group in ('RampPlatform', 'FreePlatform'):
        for shape in required_group(data, group):
            closed = shape.get('closed', False)
            name, fill, outline, width = platform_style(group, shape, outline_only)
            shape_path(shape, closed, name, fill, outline, width)
